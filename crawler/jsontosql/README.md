# YERBASODES DATABASE SCHEMA SETUP
# written by push
...
CREATE TABLE `nodes` (
  ...
  `yerbasrelease` varchar(48) NOT NULL,
...
![https://gateway.yerbasland.org/ipfs/QmYWarok4Z1au9xb9MD1dV2DmroqUfd77qtU8Z88LdqQv5](https://gateway.yerbasland.org/ipfs/QmYWarok4Z1au9xb9MD1dV2DmroqUfd77qtU8Z88LdqQv5)
...
# Author: push
# Date: 08/01/2019
# Create SQL Database of Yerbas Nodes Export Data
...
```mysql
MariaDB [yerbasstatus]> select * from nodes;
```

Querying All Database Fields from a Specific Region

```mysql
MariaDB [yerbasstatus]> select * from nodes where locality LIKE '%Europe%';
```

Querying All Database Fields from a Specific City

```mysql
MariaDB [yerbasstatus]> select * from nodes where city = 'London';
```

Querying Nodes from a specific Unix Date Range

```mysql
MariaDB [yerbasstatus]> select * from nodes where timefound LIKE '15494095%'
```
...
```mysql
MariaDB [(none)]> GRANT ALL PRIVILEGES ON yerbasstatus.* to yerbasstatus@localhost identified by 'changethiswithasecurepassword' ;                                                                                                     
```

**Create Database & Schema**

```mysql
MariaDB [(none)]> create database yerbasstatus;
...
Then test with the provided test data set that the importer is working correctly. You may replace json/1549425135.json with any output from Yerbasodes in JSON format.
